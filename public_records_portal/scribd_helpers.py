...
def get_scribd_download_url(doc_id, record_id = None):
	...
	USERNAME = app.config['SCRIBD_USERNAME']
	...
	try:
		...
		app.logger.info("\n\n Download URL: %s" %doc_url)
		if record_id:
			set_scribd_download_url(download_url = doc_url, record_id = record_id)
		return doc_url
	except Exception, e:
		app.logger.info("\n\nGetting download_url failed: %s" %e)
		return None


def set_scribd_download_url(download_url, record_id):
	app.logger.info("\n\nSetting scribd download URL...")
	db_helpers.update_obj('download_url', download_url, obj_type = 'Record', obj_id = record_id)
